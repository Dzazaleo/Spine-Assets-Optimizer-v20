...
export const GlobalStatsSummary: React.FC<GlobalStatsSummaryProps> = ({ 
  ...
  // Handle programmatic focus/highlighting
  useEffect(() => {
    if (focusedKey) {
      setIsOpen(true);
          ...
          <table className="w-full text-left border-t border-gray-700">
            ...
            <tbody className="divide-y divide-gray-800">
              {stats.map((stat) => {
                ...
                const isHighlighted = stat.lookupKey === highlightedKey;
                ...
                return (
                  <tr 
                    key={stat.lookupKey} 
                    ref={(el) => {
                      if (el) rowRefs.current.set(stat.lookupKey, el);
                      else rowRefs.current.delete(stat.lookupKey);
                    }}
                    onClick={(e) => onMultiSelect?.(stat.lookupKey, visibleKeys, e)}
                    className={clsx(
                      "transition-colors cursor-pointer select-none",
                      isHighlighted ? "bg-yellow-500/30 ring-2 ring-yellow-500 z-10" :
